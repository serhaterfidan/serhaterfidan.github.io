...
class OrderController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        $charge = 0;
        $prii = $price2;
        if ($payment_method == 'COD' || $payment_method == 'cod' || $payment_method == 'Pos Device') {
            $walletamt = 0;

            $payment_status = ($payment_method == "COD" || $payment_method == 'cod') ? 'COD' : 'Pos Device';
		...
		if ($tip!=null){
			...
			if ($payment_status == 'success' || $payment_status == 'no charge') {
				
				if(isset(explode(",",$request->payment_id)[1])){
                    $payments = DB::table('orders_payments')
                            ->insert([
                                'cart_id' => $cart_id,
                                'user_id' => $user_id,
                                'token' => explode(",",$request->payment_id)[0],
                                'reference' => explode(",",$request->payment_id)[1],
                                'card_number' => explode(",",$request->payment_id)[2],
                                'payment_gateway' => $payment_gateway,
                                'payment_id' => $payment_id,
                                'created_at' => Carbon::now(),
                                'updated_at' => Carbon::now()
                            ]);
                    $request->payment_id=explode(",",$request->payment_id)[1];
                }
				 if ($tip!=null){
                    $rem_amount = $rem_amount + $tip;
                }
                $oo = DB::table('orders')
                    ...
                    ->update([
						'order_notes' => $order_notes,
						'tip' => $tip,
						'pay_reference' => $request->payment_id,
                        'paid_by_wallet' => $walletamt,
                        'rem_price' => $rem_amount,
                        'payment_method' => $payment_method,
						'payment_status' => $request->payment_status
	...
	public function getLastOrder(Request $request)
    {
        ...
        $ongoings = DB::table('orders')
            ->whereNull('orders.deletedAt')
            ...
            ->where(function($query) use ($order_id)
            {
                if($order_id || $order_id!=null) {
                    ...
                    $query->where('orders.order_rating_state', 'on hold');
                }
            })
    ...
    public function orderCommentAction(Request $request)
    {
        ...
        if($type == "skip"){
            $oo = DB::table('orders')
                ...
                ->update([
                    'order_rating_state' => 'reject',
                ]);
            $message = array('status' => '1', 'message' => 'Skip', 'data' => []);
            ...
            $oo = DB::table('orders')
                ...
                ->where('order_id', $order_id)
                ->update([
                    'order_rating_state' => 'done',
                    'rating_value' => $ratingValue,
                    'rating_msg' => $ratingMsg,
                ]);
            $message = array('status' => '1', 'message' => 'Done', 'data' => []);

            return $message;
        }
        
    }
	public function getOrderCard(Request $request)
    {
       $user_id = $request->user_id;
       $ongoing = DB::table('orders_payments')
           ->where('orders_payments.user_id',$user_id)
           ->whereNull('orders_payments.deletedAt')
           ->groupBy('orders_payments.card_number')
           ->get();


       if (count($ongoing) > 0) {
           $message = array('status' => '1', 'message' => 'All Card', 'data' => $ongoing);
           return $message;
       }
   }
    ...
    public function ongoing(Request $request)
    {
        ...
        if (count($ongoing) > 0) {
            foreach ($ongoing as $ongoings) {
                ...
                $data[] = array('order_id'=>$ongoings->order_id,'order_rating_state' => $ongoings->order_rating_state,'user_name' => $ongoings->name, 'tip'=>$ongoings->tip, 'currency' => getCurrencyFromOrder($ongoings->order_id), 'delivery_address' => $ongoings->house_no . ',' . $ongoings->society . ',' . $ongoings->city . ',' . $ongoings->landmark . ',' . $ongoings->state . ',' . $ongoings->pincode, 'store_name' => $ongoings->store_name, 'store_owner' => $ongoings->employee_name, 'store_phone' => $ongoings->phone_number, 'store_email' => $ongoings->email, 'store_address' => $ongoings->address, 'order_status' => $ongoings->order_status, 'delivery_date' => $ongoings->delivery_date, 'time_slot' => $ongoings->time_slot, 'payment_method' => $ongoings->payment_method, 'payment_status' => $ongoings->payment_status, 'paid_by_wallet' => $ongoings->paid_by_wallet, 'cart_id' => $ongoings->cart_id, 'total_price' => $ongoings->total_price, 'delivery_charge' => $ongoings->delivery_charge, 'rem_price' => $ongoings->rem_price, 'coupon_discount' => $ongoings->coupon_discount, 'dboy_name' => $ongoings->boy_name, 'dboy_phone' => $ongoings->boy_phone, 'price_without_delivery' => $ongoings->price_without_delivery, 'avg_tax_per' => $ongoings->avg_tax_per, 'total_tax_price' => $ongoings->total_tax_price, 'user_id' => $ongoings->user_id, 'total_products_mrp' => $ongoings->total_products_mrp, 'discountonmrp' => $ongoings->discountonmrp, 'cancelling_reason' => $ongoings->cancelling_reason, 'order_date' => $ongoings->order_date, 'dboy_id' => $ongoings->dboy_id, 'user_signature' => $ongoings->user_signature, 'coupon_id' => $ongoings->coupon_id, 'dboy_incentive' => $ongoings->dboy_incentive, 'total_items' => $ongoings->total_items, 'data' => $order);
            }
            $message = array('status' => '1', 'message' => 'My All orders', 'data' => $data);
    ...
    public function whatsnew(Request $request)
    {
        ...
        if ($request->min_discount == NULL){
            $request->min_discount = 1;
        }
        if ($request->max_discount == NULL){
            $request->max_discount = 100;
        }

        $discountedProductsFilter = generateFilterCases($request);
        ...
        if (count($discountedProducts) > 0){
            $message = array('status' => '1', 'message' => 'Discounted in Store', 'data' => $discountedProducts);
            return $message;
        } else {
            $message = array('status' => '0', 'message' => 'nothing in Discounted');
            return $message;
        }
    ...
    public function completed_orders(Request $request)
    {
        ...
        if (count($ongoing) > 0) {
            foreach ($ongoing as $ongoings) {
                ...
                $data[] = array('user_name' => $ongoings->name, 'currency' => getCurrencyFromOrder($ongoings->order_id), 'delivery_address' => $ongoings->house_no . ',' . $ongoings->society . ',' . $ongoings->city . ',' . $ongoings->landmark . ',' . $ongoings->state . ',' . $ongoings->pincode, 'store_name' => $ongoings->store_name, 'store_owner' => $ongoings->employee_name, 'store_phone' => $ongoings->phone_number, 'store_email' => $ongoings->email, 'store_address' => $ongoings->address, 'order_status' => $ongoings->order_status, 'delivery_date' => $ongoings->delivery_date, 'time_slot' => $ongoings->time_slot, 'payment_method' => $ongoings->payment_method, 'payment_status' => $ongoings->payment_status, 'paid_by_wallet' => $ongoings->paid_by_wallet, 'cart_id' => $ongoings->cart_id, 'total_price' => $ongoings->total_price, 'delivery_charge' => $ongoings->delivery_charge, 'rem_price' => $ongoings->rem_price, 'coupon_discount' => $ongoings->coupon_discount, 'dboy_name' => $ongoings->boy_name, 'dboy_phone' => $ongoings->boy_phone, 'price_without_delivery' => $ongoings->price_without_delivery, 'avg_tax_per' => $ongoings->avg_tax_per, 'total_tax_price' => $ongoings->total_tax_price, 'user_id' => $ongoings->user_id, 'total_products_mrp' => $ongoings->total_products_mrp, 'discountonmrp' => $ongoings->discountonmrp, 'cancelling_reason' => $ongoings->cancelling_reason, 'order_date' => $ongoings->order_date, 'dboy_id' => $ongoings->dboy_id, 'user_signature' => $ongoings->user_signature, 'coupon_id' => $ongoings->coupon_id, 'dboy_incentive' => $ongoings->dboy_incentive, 'total_items' => $ongoings->total_items, 'data' => $order);
            }
            $message = array('status' => '1', 'message' => 'My All orders', 'data' => $data);
            return $message;
        } else {
    ...
    public function top_selling(Request $request)
    {
        ...
        $topselling = DB::table('store_products')
            ...
            ->where('store_products.stock', '>', 0)
            ->paginate(10);
        $topselling = generateProductRequirements($topselling, $request);
        $topselling = $topselling->unique('product_id');
    ...
    public function spotlight(Request $request)
    {
        ...
        if ($recentselling != NULL) {
            ...
            $recentselling = generateTranslatedProductItem($recentselling, $request);
    ...
    public function recentselling(Request $request)
    {
        ...
        if ($recentselling != NULL) {
            ...
            $lang = strtoupper($request->header('langCode'));
            $array = class2Array($recentselling);
            foreach ($array as $key => $value){
                $array[$key]['currency'] = getCurrencyData($value['store_id']);
                ...
                $array[$key]['varients'][0]['currency'] = getCurrencyData($array[$key]['varients'][0]['store_id']);
            ...
            // AKTİF DİLDE VERİYİ GETİRMEK
            $message = array('status' => '1', 'message' => 'recent selling products', 'data' => $class);
            return $message;
        } else {
            $message = array('status' => '0', 'message' => 'nothing in recentselling');
            return $message;
