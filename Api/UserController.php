...
class UserController extends Controller
{
    ...
    public function social_login(Request $request)
    {
        ...
        if ($logintype == 'google') {
            ...
            if ($checkuser) {
                ...
                $user = DB::table('users')->whereNull('deletedAt')->where([['email', '=', $user_email]])->first();
                ...
                $Userreg = DB::table('users')->insertGetId(['email' => $user_email, 'name' => 'User', 'is_verified' => 0]);
            ...
            if ($checkuser) {
                $updateDeviceId = DB::table('users')
                    ...
                    ->orWhere('email', $email)
                    ->update(['device_id' => $device_id]);
                $user = DB::table('users')->whereNull('deletedAt')->where([['facebook_id', '=', $fb_id]])->orWhere([['email', '=', $email]])->first();
                $token = $user->createToken('token')->accessToken;
                $user_id = $user->id;
                $sum = DB::table('store_orders')
                    ->whereNull('deletedAt')
    ...
    public function login(Request $request)
    {
        ...
        $checkUser = DB::table('users')->where([['user_phone', '=', $user_phone],['is_verified', '=', 1],['deletedAt', '=', NULL]])->first();
        ...
        if ($checkUser) {
            ...
            $unvuser = DB::table('users')
                ...
                ->where('is_verified', 0)
                ->first();
            $unvuser = DB::table('users')->where([['user_phone', '=', $user_phone],['is_verified', '=', 0],['deletedAt', '=', NULL]])->first();
    ...
    public function register_details(Request $request)
    {
        ...
        if ($fb_id == NULL) {
            $check = DB::table('users')
                ->where([['user_phone', '=', $user_phone],['deletedAt', '=', NULL]])
                ->orWhere([['email', '=', $user_email],['deletedAt', '=', NULL]])
                ->first();
        } else {
            $check = DB::table('users')
                ->where([['user_phone', '=', $user_phone],['deletedAt', '=', NULL]])
                ->orWhere([['email', '=', $user_email],['deletedAt', '=', NULL]])
                ->orWhere([['facebook_id', '=', $fb_id],['deletedAt', '=', NULL]])
                ->first();
        }
    ...
    public function login_with_email(Request $request)
    {
        ...
        $device_id = $request->device_id;
        $checkUser = DB::table('users')->where([['email', '=', $user_email],['deletedAt', '=', NULL]])->first();
    ...
    public function deleteAccount(Request $request){
        ...
        foreach ($orders as $order){
            DB::table('store_orders')
                ->where([['order_cart_id', '=', $order->cartID]])
                ->update(['deletedAt' => $time]);
        }

        // Müşteri favorilerini silmek
        DB::table('wishlist')
            ->where([['user_id', '=', $userID]])
            ->update(['deletedAt' => $time]);

        // Cüzdan doldurma geçmişi silmek
        DB::table('wallet_recharge_history')
            ->where([['user_id', '=', $userID]])
            ->update(['deletedAt' => $time]);

        // Müşteri son aramalarını silmek
        DB::table('recent_search')
            ->where([['user_id', '=', $userID]])
            ->update(['deletedAt' => $time]);

        // Müşteri adreslerini silmek
        DB::table('address')
            ->where([['user_id', '=', $userID]])
            ->update(['deletedAt' => $time]);

        // Müşteri Bildirimlerini silmek
        DB::table('user_notification')
            ->where([['user_id', '=', $userID]])
            ->update(['deletedAt' => $time]);

        // Müşteri foto siparişlerini silmek
        DB::table('order_by_photo')
            ->where([['user_id', '=', $userID]])
            ->update(['deletedAt' => $time]);
        ...
        if ($delete){
            ...
            return data2JSON([
                ...
                'data' => [
                    'message' => 'An Error Occured on Progress.',
                ],
            ]);
        }
    }
