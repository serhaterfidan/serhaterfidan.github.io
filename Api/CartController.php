...
class CartController extends Controller
{
    ...
    public function add_to_cart(Request $request){
        ...
        $activeLang = strtoupper($request->header('langCode'));
        $userID = $request->user_id;
        $storeID = $request->store_id;
        $qty = $request->qty;
        $varientID = $request->varient_id;
        $dateNow = Carbon::Now();
        ...
        // Sepetteki ürünleri almadan önce temizlemek
        DB::table('store_orders')->whereNotNull('deletedAt')->where([['order_cart_id', '=', 'incart']])->delete();
        // Sepetteki ürünleri almak
        $cartItemCheck = DB::table('store_orders')->whereNull('deletedAt')->where([['store_approval', '=', $userID],['order_cart_id', '=', 'incart']])->first();


        // Sepette Ekli Ürün Var mı?
        if ($cartItemCheck){
            // Eğer mağaza idleri farklı ise sepeti boşaltmak
            $lastStoreID = $cartItemCheck->store_id;
            if ($storeID != $lastStoreID) {
                DB::table('store_orders')->where([['order_cart_id', '=', 'incart'],['store_approval', '=', $userID]])->delete();
            }
        }

        // Kullanıcıyı Almak
        $user = DB::table('users')->select('user_phone', 'wallet')->where([['id', '=', $userID]])->first();
        if (!$user) {
        ...
        // Eğer ürün silinmeye çalışılıyorsa 0 ile istek geliyorsa
        if ($qty == 0){
            return $this->removedCart($request);
        }

        // İstekte bulunulan ürünü getirmek
        $storeProduct = DB::table('store_products')
            ->whereNull('store_products.deletedAt')
            ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
            ->join('product', 'product_varient.product_id', '=', 'product.product_id')
            ->where('store_products.varient_id', $varientID)
            ->where('store_products.store_id', $storeID)
            ->first();
        $isSpecial = $storeProduct->isSpecial;

        // Stoklarda var mı diye kontrol etmek
        $maxOrderQTY = ($isSpecial) ? $storeProduct->max_ord_qty/10 : $storeProduct->max_ord_qty;;
        $minOrderQTY = ($isSpecial) ? $storeProduct->min_ord_qty/10 : $storeProduct->min_ord_qty;;
        $quantity =  $storeProduct->quantity;
        $unit = lngUnit($storeProduct->unit, $activeLang);
        $stock = $storeProduct->stock;
        $productName = tPC($storeProduct->product_name, $activeLang);
        $wantedQTY = ($isSpecial) ? $qty/10 : $qty;
        $productNameWithUnitAndQTY = $productName . "(" . $quantity . $unit . ")*" . $qty;
        $productImage = $storeProduct->product_image;


        // Maximum alınabilen adetten fazla mı diye kontrol etmek
        if ($wantedQTY > $maxOrderQTY) {
            $message = array('status' => '0', 'message' => 'you have to order ' . $productNameWithUnitAndQTY . ' quantity between ' . $minOrderQTY . ' to ' . $maxOrderQTY);
            return $message;
        }
        // Minimum alınabilen adetten az mı diye kontrol etmek
        if ($wantedQTY < $minOrderQTY) {
            $message = array('status' => '0', 'message' => 'you have to order ' . $productNameWithUnitAndQTY . ' quantity between ' . $minOrderQTY . ' to ' . $maxOrderQTY);
            return $message;
        }
        // Stok var mı diye kontrol etmek
        if ($stock < $qty) {
            $message = array('status' => '0', 'message' => 'only ' . $stock . ' available in stock');
            return $message;
        }


        // Tüm ürün fiyatlarını vs hesaplatmak
        $calculatePricesForSingleProduct = calculatePricesForSingleProduct($storeProduct, $storeID, $varientID, $dateNow, $wantedQTY);
        $productPrice = $calculatePricesForSingleProduct->productPrice;
        $productMRP = $calculatePricesForSingleProduct->productMRP;
        $totalProductPrice = $calculatePricesForSingleProduct->totalProductPrice;
        $totalProductMRP = $calculatePricesForSingleProduct->totalProductMRP;
        $categoryTax = $calculatePricesForSingleProduct->categoryTax;
        $categoryTaxType = $calculatePricesForSingleProduct->categoryTaxType;
        $categoryTaxName = $calculatePricesForSingleProduct->categoryTaxName;
        $totalProductTax = $calculatePricesForSingleProduct->totalProductTax;
        $totalProductPriceWithoutTax = $calculatePricesForSingleProduct->totalProductPriceWithoutTax;
        $totalProductPriceWithTax = $calculatePricesForSingleProduct->totalProductPriceWithTax;

        // Zaten ekli mi diye kontrol etmek
        $productCartCheck = DB::table('store_orders')->whereNull('deletedAt')->where([['store_approval', '=', $userID],['varient_id', '=', $varientID],['order_cart_id', '=', 'incart']])->first();
        $insertData = [
            'store_id' => $storeID,
            ...
            'total_mrp' => $totalProductMRP,
            'order_cart_id' => "incart",
            'order_date' => $dateNow,
            'price' => $totalProductPriceWithTax,
            'description' => $storeProduct->description,
            'tx_per' => ($totalProductTax != 0) ? $totalProductTax/$wantedQTY : 0,
            'price_without_tax' => $totalProductPriceWithoutTax,
            'tx_price' => $totalProductTax,
            'tx_name' => $categoryTaxName,
            'type' => $storeProduct->type
        ];

        // Ekli değilse ve ekliyse burada ekleme işlemi yapılacak
        if (!$productCartCheck) {
            $insert = DB::table('store_orders')->insert($insertData);
        } else {
            $delete = DB::table('store_orders')->where([['store_approval', '=', $userID],['varient_id', '=', $varientID],['order_cart_id', '=', 'incart']])->delete();
            $insert = DB::table('store_orders')->insert($insertData);
        }


        if ($insert) {
            $delete = DB::table('store_orders')->where([['store_approval', '=', $userID],['varient_id', '=', $varientID],['qty', '=', '0']])->delete();

            $totalPricesSum = DB::table('store_orders')
                ->whereNull('deletedAt')
            ...
            $cartItems = generateCartItems($cartItems, $request);

            $storeData = DB::table('store')->where([['id', '=', $storeID]])->first();
            $storeData = class2Array($storeData);
            $storeData['currency'] = getCurrencyData($storeData['id']);
            $storeData = array2Class($storeData);

            $adata = array(
                'discountonmrp' => $totalPricesSum->mrp-$totalPricesSum->sum,
                'total_price' => $totalPricesSum->sum,
                'total_mrp' => $totalPricesSum->mrp,
                'total_items' => $totalPricesSum->count,
                'store_details' => $storeData,
                'total_tax' => $totalPricesSum->sum_tax,
                'avg_tax' => $taxPerAVG,
                'data' => $cartItems
            );
    ...
    public function make_an_order(Request $request)
    {
       ...
       if (!$typessss) {
        ...
        $data_array = DB::table('store_orders')
            ...
            ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
            ->join('product', 'product_varient.product_id', '=', 'product.product_id')
            ->select('store_orders.*', 'product.isSpecial',)
        ...
        foreach ($data_array as $h) {
            ...
            $p = DB::table('store_orders')
                ...
                ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                ->join('product', 'product_varient.product_id', '=', 'product.product_id')
                ->select('store_orders.*', 'product.cat_id', 'product.isSpecial',)
                ->where('store_orders.order_cart_id', 'incart')
        ...
        foreach ($data_array as $h) {
            ...
            $p = DB::table('store_orders')
                ...
                ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                ->join('product', 'product_varient.product_id', '=', 'product.product_id')
                ->select('store_orders.*', 'product.isSpecial', 'store_products.min_ord_qty', 'store_products.max_ord_qty', 'product.cat_id')
                ->where('store_orders.varient_id', $varient_id)
                ->where('store_orders.store_id', $store_id)
                ->where('store_orders.order_cart_id', 'incart')
                ->where('store_orders.store_approval', $user_id)
            ...
            $unit[] = $p->unit;
            $qty[] = $p->quantity;
            $order_qty2 = (getProductIsSpecial($varient_id)) ? $order_qty/10 : $order_qty;
            $p_name[] = tPC($p->product_name, 'AZ') . "(" . $p->quantity . ' ' . lngUnit($p->unit, 'AZ') . ")*" . $order_qty2;
        ...
        if ($insert) {
            ...
            $ordersuccessed = class2Array($ordersuccessed);
            $ordersuccessed['currency'] = getCurrencyData($ordersuccessed['store_id']);
            $ordersuccessed = array2Class($ordersuccessed);
    ...
    public function show_cart(Request $request){
        ...
        // Yeni güncelleme ile sepette mağaza id değişince ürünleri temizlemek
        if ($request->store_id){
            // Sepetteki ürünleri almak
            $cartItemCheck = DB::table('store_orders')->whereNull('deletedAt')->where([['store_approval', '=', $userID],['order_cart_id', '=', 'incart']])->first();
        ...
        // Ürün var mı diye kontrol etmek
        $cartItemCheck = DB::table('store_orders')->whereNull('deletedAt')->where([['store_approval', '=', $userID],['order_cart_id', '=', 'incart']])->first();

        if ($cartItemCheck){
            $storeData = DB::table('store_orders')->whereNull('deletedAt')->where([['store_approval', '=', $userID],['order_cart_id', '=', 'incart']])->first();
            $totalPricesSum = DB::table('store_orders')->whereNull('store_orders.deletedAt')
                ->join('store_products', 'store_orders.varient_id', '=', 'store_products.varient_id')
                ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                ->join('product', 'product_varient.product_id', '=', 'product.product_id')
                ->where('store_products.store_id', $storeData->store_id)
                ->where('store_orders.store_approval', $userID)
				->whereNull('store_products.deletedAt')
                ->where('store_orders.order_cart_id', 'incart')
                ->select(DB::raw('SUM(store_orders.total_mrp) as mrp'), DB::raw('SUM(store_orders.price) as sum'), DB::raw('COUNT(store_orders.store_order_id) as count'), DB::raw('SUM(store_orders.tx_price) as sum_tax'), DB::raw('SUM(store_orders.tx_per) as sum_per'))
            ...
            $totalTaxAVG = $totalPricesSum->sum_per / $totalPricesSum->count;
            $totalTax = $totalPricesSum->sum_tax;
            ...
            $storeData2 = DB::table('store')->where('id', $storeData->store_id)->first();
            $storeData2 = class2Array($storeData2);
            $storeData2['currency'] = getCurrencyData($storeData2['id']);
            $storeData2 = array2Class($storeData2);
    ...
    public function removedCart(Request $request){
        ...
        if ($cartItemCheck){
            ...
            $message = array('status' => '1', 'message' => 'Product has been removed from cart', 'data' => $adata);
            return $message;
    ...
    public function re_ordercart(Request $request)
    {
        ...
        if ($insert) {
            ...
            $cart_items1 = DB::table('store_orders')
                ...
                ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                ->join('product', 'product_varient.product_id', '=', 'product.product_id')
                ->select('store_orders.product_name', 'product.isSpecial', 'store_orders.varient_image', 'store_orders.quantity', 'store_orders.unit', 'store_products.price', 'store_orders.qty', 'store_orders.total_mrp', 'store_orders.order_cart_id', 'store_orders.order_date', 'store_orders.store_approval', 'store_orders.store_id', 'store_orders.varient_id', 'product.product_id', 'store_products.stock', 'store_orders.tx_per', 'store_orders.price_without_tax', 'store_orders.tx_price', 'store_orders.tx_name', 'product.product_image', 'product_varient.description', 'product.type')
            ...
            $nearbystore = class2Array($nearbystore);
            $nearbystore['currency'] = getCurrencyData($nearbystore['id']);
            $nearbystore = array2Class($nearbystore);
